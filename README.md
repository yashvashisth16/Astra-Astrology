...
A premium, single-page astrological consulting website built with pure HTML, CSS, and vanilla JavaScript. Designed with a dark-first aesthetic, smooth animations, and a fully validated consultation booking form.
...
- **Consultation booking form** — fully validated with real-time inline error messages including:
  - Full Name, Email (optional), Date of Birth, Time of Birth
...
- **Horizontal scrolling case studies** — draggable portfolio rail with three client stories
...
├── script.js       # Theme toggle, form validation, scroll reveals, counter animations
└── README.md       # This file
```
...
| `--text-primary` | `#f0ede6` | Main body text (dark mode) |
...
| Email | Optional — validated against RFC-style regex if provided |
...
Design tokens are defined as CSS custom properties at the top of `styles.css` inside `:root` (dark mode) and `[data-theme="light"]`. Update these to retheme the entire site.
