...
export default function VideoPlayer({ src, delay }: { src: string, delay: number }) {
  ...
  return (
    <div className={`video-wrapper reveal scale-up delay-${delay}`}>
      <video 
        ref={videoRef}
        src={src} 
      ...
      <div className="video-controls">
        <button className="control-btn play-pause-btn" aria-label={isPlaying ? "Pause" : "Play"} onClick={togglePlay}>
          {isPlaying ? iconPause : iconPlay}
        </button>
        <button className="control-btn mute-unmute-btn" aria-label={isMuted ? "Unmute" : "Mute"} onClick={toggleMute}>
